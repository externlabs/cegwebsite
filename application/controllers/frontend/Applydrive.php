<?php
class Applydrive extends CI_controller
{
    ...
    public function index()
    {
       ...
       $this->load->view('frontend/template/navbar');
        $this->load->view('frontend/applydrive');
    ...
    public function apply()
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $drive_application = $this->db->where('drive_id',$driveId)->where('student_id',$student_id)->get('drive_application')->result_array();

            if($drive_application == Array()){
                $datas = array(
                    'term' => "agree",
                    'qualification' => "agree",
                    'eligible' => "agree",
                ...
                $this->session->set_flashdata('error', 'You Have Already Applied In this Drive');
                redirect(base_url() . 'apply/'.$driveId);
            } 
        } else {
